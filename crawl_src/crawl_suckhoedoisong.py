...
URL_LIST = [
    "https://suckhoedoisong.vn/timelinelist/1691470/"
]

def get_links(url:str):
    soup = get_html(url)
    titles = soup.findAll("a", {"class":"box-category-link-title"})
    links = []
    for title in titles:
        links.append("https://suckhoedoisong.vn/" + title['href'])
...
def get_date(soup):
    datetime = soup.find('span', {'class':'publish-date'}).text
...
def get_title(soup):
    title = soup.find('h1', {'class':'detail-title'}).text
